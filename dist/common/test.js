...
var Test = {
        indicesWalkTest: function indicesWalkTest() {
                ...
                { x: 0, y: 0 }, { x: 22, y: 20 }, { x: 20, y: 50 }, { x: -20, y: 50 }];

                var e_verts = [// in clockwise sorted order
                { x: 0, y: -30 }, { x: 20, y: -30 }, { x: 22, y: 20 }, { x: 0, y: 0 }];
                ...
                console.assert(d.vertices[0].x == d_verts[0].x && d.vertices[0].y == d_verts[0].y, "Indices don't match on body 'd'.");

                console.assert(e.vertices[0].x == e_verts[0].x && e.vertices[0].y == e_verts[0].y, "Indices don't match on body 'e'.");

                result = walk_indices(body, d, 0, 1);
                console.assert(result.part.id === e.id && result.index === 3, "d0 1 part id: " + result.part.id + "(=>" + e.id + ") index: " + result.index + "(=>" + 3 + ")");

                result = walk_indices(body, e, 3, -1);
                console.assert(result.part.id === d.id && result.index === 0, "e3 -1 part id: " + result.part.id + "(=>" + d.id + ") index: " + result.index + "(=>" + 0 + ")");

                result = walk_indices(body, d, 1, -1);
                console.assert(result.part.id === e.id && result.index === 2, "d1 -1 part id: " + result.part.id + "(=>" + e.id + ") index: " + result.index + "(=>" + 2 + ")");

                result = walk_indices(body, e, 2, 1);
                console.assert(result.part.id === d.id && result.index === 1, "e2 1 part id: " + result.part.id + "(=>" + d.id + ") index: " + result.index + "(=>" + 1 + ")");
        },

        bodyWalkTest: function bodyWalkTest() {
                ...
                console.assert(result.distance === 100 && result.walk[result.walk.length - 1].x === 50 && result.walk[result.walk.length - 1].y === 50, result);
                ...
                console.assert(result.distance === 200 && result.walk[result.walk.length - 1].body.id === b.id && result.walk[result.walk.length - 1].x === 150 && result.walk[result.walk.length - 1].y === -50, result);

                result = Matter.Body.walk(body, b, { x: 150, y: -50 }, -200);
                console.assert(result.distance === 200 && result.walk[result.walk.length - 1].x === -50 && result.walk[result.walk.length - 1].y === -50, result);

                result = Matter.Body.walk(body, b, { x: 150, y: 50 }, 200);
                console.assert(result.distance === 200 && result.walk[result.walk.length - 1].x === -50 && result.walk[result.walk.length - 1].y === 50, result);

                result = Matter.Body.walk(body, a, { x: -50, y: 50 }, -200);
                console.assert(result.distance === 200 && result.walk[result.walk.length - 1].x === 150 && result.walk[result.walk.length - 1].y === 50, result);

                result = Matter.Body.walk(body, a, { x: -50, y: 0 }, -200);
                console.assert(result.distance === 200 && result.walk[result.walk.length - 1].x === 100 && result.walk[result.walk.length - 1].y === 50, result);
                ...
                console.assert(result.distance === 200 && result.walk[result.walk.length - 1].x === 150 && result.walk[result.walk.length - 1].y === 100, result);
        }
};